...
% 设置绘图参数
y_label = 'SNR';
x_label = 'Mean connectivity alteration';
y_range = [0 10];
y_ticks = [0 2 4 6 8 10];
x_range = [-0.3 0.3];
x_ticks = [-0.3 -0.2 -0.1 0 0.1 0.2 0.3];
output_folder = 'D:/ssvepnet/aveconnectstrength/';
...
% 
%     %获得平均连接改变量
%     filename2 = ['D:/ssvepnet/aveconnectstrength/sub_all_aveconnectstrength/aveconectstrength_', num2str(state), '.csv'];
%     data2 = csvread(filename2);
%     rie_data = data2(:,1);
% 
%     % 创建散点图
%     fig = figure('Visible', 'off');
%     scatter(rie_data, snr_data, "square", 'MarkerEdgeColor', [0.522 0.007 0.243]);
%     hold on;
...
% 
%     % 保存图像为JPEG文件
%     saveas(fig, [output_folder, 'alter_state', num2str(state), '.jpg'], 'jpg');
%     close(fig);
% end
...
for state = 1:27
    % 获取当前状态的信噪比数据
    filename1 = ['D:/ssvepnet/sub_all_snr/sub_all_snr_', num2str(state), '.csv'];
    data3 = csvread(filename1);
    snr_data2(:,state) = data3(:,1); 
    
    %获得平均连接改变量
    filename2 = ['D:/ssvepnet/aveconnectstrength/sub_all_aveconnectstrength/aveconectstrength_', num2str(state), '.csv'];
    data4 = csvread(filename2);
    rie_data2(:,state) = data4(:,1);
end
...
for i=1:3
    ...
    % 创建散点图
    fig = figure('Visible', 'off');
    scatter(rie_current, snr_current, "square", 'MarkerEdgeColor', [0.522 0.007 0.243]);
    hold on;
...
%   plot(rie_data, snr_data);
    ...
    % 保存图像为JPEG文件
    saveas(fig, [output_folder, 'avecon_', fre_moji{i}, '.jpg'], 'jpg');
    close(fig);
end
...
%   plot(rie_data, snr_data);
    ...
    % 保存图像为JPEG文件
    saveas(fig, [output_folder, 'avecon_', col_moji{i}, '.jpg'], 'jpg');
    close(fig);
end
...
%   plot(rie_data, snr_data);
    ...
    % 保存图像为JPEG文件
    saveas(fig, [output_folder, 'avecon_', lum_moji{i}, '.jpg'], 'jpg');
    close(fig);
end