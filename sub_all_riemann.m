...
for file_num = 1:27
    ...
    for sub = 1:length(subjects)
        % 读取csv文件
        file_path = ['D:/ssvepnet/', subjects{sub}, '/riemann/riemannian_', num2str(file_num), '.csv'];
        data(sub) = readmatrix(file_path);
    end
    
    % 将数据写入新的csv文件
    new_file_path = ['D:/ssvepnet/sub_all_riemann/sub_all_riemann_', num2str(file_num), '.csv'];
